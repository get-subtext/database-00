...
import type { OpenSubtitlesMapper, SubtitlePackage } from './OpenSubtitlesMapper.types';
...
export class OpenSubtitlesMovieReader implements T.MovieReader {
  ...
  private async getSubtitlePackageList(imdbId: string) {
    type TOutput = { data: SubtitlePackage[]; logs: FetchLog[] };
    const output: TOutput = { data: [], logs: [] };

    const subtitlesFirstPage = await this.openSubtitlesApi.getSubtitlesPage(imdbId, 1);
    output.logs.push(subtitlesFirstPage.log);

    if (subtitlesFirstPage.success) {
      const subtitlePackagePage = this.openSubtitlesMapper.toSubtitlePackagePage(subtitlesFirstPage.data);
      output.data.push(...subtitlePackagePage.subtitlePackages);

      const totalPages = subtitlePackagePage.totalPages ?? 1;
      const remainingPages = range(2, totalPages + 1);
      const getSubtitlesPromises = map(remainingPages, (page) => this.openSubtitlesApi.getSubtitlesPage(imdbId, page));
      ...
      for (let j = 0; j < getSubtitlesResults.length; j++) {
        ...
        if (getSubtitlesRes.success) {
          const subtitlePackagePage = this.openSubtitlesMapper.toSubtitlePackagePage(getSubtitlesRes.data);
          output.data.push(...subtitlePackagePage.subtitlePackages);
        }
      }
    }
  ...
  private async toMovie({ title, releaseYear, posterUrl, author, files }: SubtitlePackage) {
